...
async function mintNFTWithMetaplexCore(walletAddress, metadata, metadataUrl) {
  try {
    ...
    console.log(`💰 Creator wallet balance: ${balanceInSOL} SOL`)

    if (balanceInSOL < 0.01) {
      throw new Error(
        `Creator wallet needs SOL funding. Current balance: ${balanceInSOL} SOL. Please send at least 0.01 SOL to: ${creatorKeypair.publicKey.toString()}`,
      )
    }
    ...
    if (error.message.includes("Attempt to debit an account but found no record of a prior credit")) {
      ...
      const balanceInSOL = balance / 1000000000
      throw new Error(
        `Creator wallet insufficient balance. Current: ${balanceInSOL} SOL. Need at least 0.01 SOL for transaction fees. Send SOL to: ${creatorKeypair.publicKey.toString()}`,
      )
    }
